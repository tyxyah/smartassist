...
} = require("../utils/updatePrerequisiteStatus");
  ...
  try {
    ...
    await Promise.all(
      userCourses.map(async (course) => {
        // Check if the course can be updated based on the prerequisite
        updatePrerequisiteStatus(course.course_code, userCourses);
        console.log(
          `Updating status for ${course.course_code}, Status: ${course.status}`
        );
        await course.save();
      })
    );
