...
async function runCourseSuggestion(modelName, currentSemester) {
  try {
    ...
    const failedCourses = suggestFailedCoursesModule.suggestFailedCourses(allData);

    console.log(`Failed Courses to Repeat in Semester ${currentSemester}:`, failedCourses);
  } catch (error) {
